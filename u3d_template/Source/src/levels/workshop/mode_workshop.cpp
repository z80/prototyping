...
// These are local events used only internally.
...
WorkshopMode::WorkshopMode(Context* context)
    : ModeBase(context),
      mode( None ),
      mouseX(0),
      mouseY(0),
...
void WorkshopMode::deactivate()
{
    ...
    p = root->GetChild( "Mode", true );
    if ( p )
        p->Remove();

    p = root->GetChild( "AuxPanel", true );
    if ( p )
        p->Remove();
}

void WorkshopMode::CreateScene()
{
    if ( rootNode )
        return;
...
void WorkshopMode::createTechPanel()
{
    UI * ui = GetSubsystem<UI>();
    UIElement * root = ui->GetRoot();
    UIElement * pnl = root->GetChild( "TechPanel", true );
...
void WorkshopMode::createSectionsUi()
{
    UI * ui = GetSubsystem<UI>();
    UIElement * root = ui->GetRoot();
    ...
    for ( size_t i=0; i<qty; i++ )
    {
        ...
        SubscribeToEvent( b, E_RELEASED,
            std::bind( &WorkshopMode::HandlePanelGroupClicked,
                       this, i ) );
    }
...
void WorkshopMode::createBlocksUi( int groupInd )
{
    std::vector<CategoryDesc> & cats = techTree->getPanelContent();
    const size_t qty = cats.size();
    if ( groupInd >= qty )
    ...
    for ( size_t i=0; i<typesQty; i++ )
    {
        ...
        p->AddChild( b );
        SubscribeToEvent( b, E_RELEASED,
                          std::bind( &WorkshopMode::HandlePanelBlockClicked,
                                     this, pd.name ) );
    }
}

void WorkshopMode::createModeUi()
{
    ...
    Text * t = root->CreateChild<Text>( "Mode" );
    t->SetStyleAuto();
    t->SetEditable( false );
...
void WorkshopMode::SubscribeToEvents()
{
    SubscribeToEvent(E_PHYSICSPRESTEP_2, URHO3D_HANDLER( WorkshopMode, HandlePhysicsPreStep) );
    SubscribeToEvent(E_POSTUPDATE,       URHO3D_HANDLER( WorkshopMode, HandlePostUpdate) );

    SubscribeToEvent( E_MOUSEBUTTONDOWN, URHO3D_HANDLER( WorkshopMode, HandleMouseDown ) );
    SubscribeToEvent( E_MOUSEBUTTONUP,   URHO3D_HANDLER( WorkshopMode, HandleMouseUp ) );
    SubscribeToEvent( E_MOUSEMOVE,       URHO3D_HANDLER( WorkshopMode, HandleMouseMove ) );
    SubscribeToEvent( E_KEYDOWN,         URHO3D_HANDLER( WorkshopMode, HandleKeyDown ) );
    SubscribeToEvent( E_KEYUP,           URHO3D_HANDLER( WorkshopMode, HandleKeyUp ) );

    SubscribeToEvent( E_CATEGORY_CLICKED,     URHO3D_HANDLER( WorkshopMode, HandlePanelGroupSelected ) );
    SubscribeToEvent( E_CREATE_BLOCK_CLICKED, URHO3D_HANDLER( WorkshopMode, HandlePanelBlockSelected ) );
}

Button* WorkshopMode::CreateButton(const String& text, int width, IntVector2 position)
{
...
void WorkshopMode::mouseIntersection( Vector3 & at, const Vector3 & origin )
{
...
void WorkshopMode::hintSelected()
{
...
void WorkshopMode::createAuxilaryPanel()
{
    ResourceCache * cache = GetSubsystem<ResourceCache>();
...
void WorkshopMode::deleteSelectedBlock()
{
    if ( !selectedBlock )
        return;
...
void WorkshopMode::HandlePanelGroupClicked( int ind )
{
    VariantMap & data = GetEventDataMap();
    data[ "index" ] = ind;
...
void WorkshopMode::HandlePanelBlockClicked( const String name )
{
    VariantMap & data = GetEventDataMap();
    data[ "name" ] = name;
...
void WorkshopMode::HandlePanelGroupSelected( StringHash eventType, VariantMap & eventData )
{
...
void WorkshopMode::HandlePanelBlockSelected( StringHash eventType, VariantMap & eventData )
{
...
void WorkshopMode::HandleSaveDesignDialog( StringHash eventType, VariantMap & eventData )
{
    UI * ui = GetSubsystem<UI>();
    UIElement * root = ui->GetRoot();
    ...
    if ( !e )
    {
        ...
        if ( okBtn )
            SubscribeToEvent( okBtn, E_RELEASED, URHO3D_HANDLER( WorkshopMode, HandleSaveDesignOk ) );
        UIElement * cancelBtn = e->GetChild( "Cancel", true );
        if ( cancelBtn )
            SubscribeToEvent( cancelBtn, E_RELEASED, URHO3D_HANDLER( WorkshopMode, HandleSaveDesignCancel ) );
...
void WorkshopMode::HandleSaveDesignOk( StringHash eventType, VariantMap & eventData )
{
    UI * ui = GetSubsystem<UI>();
    UIElement * root = ui->GetRoot();
    UIElement * e = root->GetChild( "DesignSave", true );
...
void WorkshopMode::HandleSaveDesignCancel( StringHash eventType, VariantMap & eventData )
{
    UI * ui = GetSubsystem<UI>();
    UIElement * root = ui->GetRoot();
    UIElement * e = root->GetChild( "DesignSave", true );
...
void WorkshopMode::HandleTry( StringHash eventType, VariantMap & eventData )
{
...
void WorkshopMode::HandleOpenDesignDialog( StringHash eventType, VariantMap & eventData )
{
    UI * ui = GetSubsystem<UI>();
    UIElement * root = ui->GetRoot();
    ...
    if ( !e )
    {
        ...
        UIElement * okBtn = e->GetChild( "Ok", true );
        if ( okBtn )
            SubscribeToEvent( okBtn, E_RELEASED, URHO3D_HANDLER( WorkshopMode, HandleOpenDesignOk ) );
        UIElement * cancelBtn = e->GetChild( "Cancel", true );
        if ( cancelBtn )
            SubscribeToEvent( cancelBtn, E_RELEASED, URHO3D_HANDLER( WorkshopMode, HandleOpenDesignCancel ) );
    }
    ...
    for ( size_t i=0; i<qty; i++ )
    {
        ...
        SubscribeToEvent( e, E_CLICK, URHO3D_HANDLER( WorkshopMode, HandleDesignSelected ) );
    }

    e->SetVisible( true );
}

void WorkshopMode::HandleDesignSelected( StringHash eventType, VariantMap & eventData )
{
    UI * ui = GetSubsystem<UI>();
    UIElement * root = ui->GetRoot();
    UIElement * e = root->GetChild( "DesignOpen", true );
...
void WorkshopMode::HandleOpenDesignOk( StringHash eventType, VariantMap & eventData )
{
    UI * ui = GetSubsystem<UI>();
    UIElement * root = ui->GetRoot();
    UIElement * e = root->GetChild( "DesignOpen", true );
...
void WorkshopMode::HandleOpenDesignCancel( StringHash eventType, VariantMap & eventData )
{
    UI * ui = GetSubsystem<UI>();
    UIElement * root = ui->GetRoot();
    UIElement * e = root->GetChild( "DesignOpen", true );
